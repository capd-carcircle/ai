...
- 지금 생성하는 질문들끼리 같은 임상 주제·증상을 중복해서 묻지 마세요. 예를 들어 "두통 있었나요?"와
  "어지러움 있었나요?"를 각각 별도 질문으로 만들지 말고, 같은 소견(예: 혈압 상승)에서 나온 관련
  증상들은 "두통이나 어지러움, 눈 침침함 같은 증상이 있었나요?"처럼 하나의 질문으로 합치세요.
  서로 다른 임상 소견(예: 혈압 상승 vs 체중 증가 vs 제수량 감소)에 대해서만 별도 질문을 만드세요.
